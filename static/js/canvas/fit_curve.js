...
var error_bezier_fit = 10;
function drawn_line_to_bezier(d, fit=false){
    fit  = fit || error_bezier_fit
    //a = d.match(/-?\d+\.?\d*,-?\d+\.?\d*/g).map(d=>d.split(',').map(d=>parseFloat(d)));
    a = d.match(/-?\d+\.?\d*[, ]-?\d+\.?\d*/g).map(d=>d.split(/[, ]/).map(d=>parseFloat(d)));
    var ps = fitCurve(a, fit)
    ps = ps.map(d=>d.map(e=>e.map(f=>round_to(f,2))))
    return fittedCurveDataToPathString(ps);
}
